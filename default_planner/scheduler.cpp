...
void schedule_plan_flow_time_expanded(int time_limit, std::vector<int> & proposed_schedule,  SharedEnvironment* env, std::vector<Double4> background_flow, bool use_traffic, bool new_only, int num_network_timesteps, unordered_map<int, vector<int>> delivery_agent_paths)
{
    ...
    // Create the graph
    int num_time_layers = num_network_timesteps + 1;
    ...
    vector<vector<NodePair>> time_expanded_map(num_time_layers, 
    ...
    for (auto task: task_loc_ids)
    {
      ...
      for(int t = 0; t < num_time_layers; t++){
        ListDigraph::Arc a = g.addArc(time_expanded_map[t][loc].second, task_loc_sink);
        capacity[a] = t == num_time_layers - 1 ? tasks_at_loc.size() : 1; // Last layer has capacity for static map flow network
        cost[a] = 0;
      }
    ...
    for (int loc = 0; loc < env->map.size(); loc++)
    {
        ...
        for (int t = 0; t < num_time_layers - 1; t++){
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            ListDigraph::Arc c = g.addArc(time_expanded_map[num_time_layers-1][loc].second, 
            time_expanded_map[num_time_layers-1][neighbor_loc].second);
            cost[c] = 1;
